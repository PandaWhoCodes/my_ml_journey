...
# shelve for savinf the files
import tflearn
# API layer over tensorflow
...
def Make_Changes(label):
    """
    Stores male or female to the persistant storage
    :param label: Stores male or female
    :return: none
    """
...
def get_images(path):
    ...
    :param path: /dataset
    :return: all the images and their labels
    """
    ...
    for img in os.listdir(path):
        ...
        # extracting the male or female part of the file from its filename using regex
        ...
        try:
            ...
            exit()
        count = count + 1
        # Adding the label to the data shelve
        Make_Changes(labl)
...
def net(X, Y, save_model=False):
    ...
    #else:
    model.fit({'input': X}, {'target': np.array(np.eye(len(set(Y)) + 1)[Y])}, n_epoch=50, batch_size=50,
              validation_set=0.1,
...
def recognize_video(face_recognizer):
    ...
    while True:
        if cap.grab():
            ...
            # using the face classifier to detect faces
            ...
            for x, y, w, h in faces:
                ...
                try:
                    ...
                    continue
                test_img[0, :, :] = img
                # and test the faces against our classifier to get 0 or 1
                ...
                if name == "male":
                    ...
                    color = (255, 0 ,255)
                cv2.rectangle(image, (x - 5, y - 5), (x + w + 5, y + h + 5), color, 2)
                cv2.putText(image, name, (x, y - 10), FONT, 0.5, color, 1)
